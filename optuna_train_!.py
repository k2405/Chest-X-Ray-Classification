...
def objective(trial):
    ...
    collor_jitter = trial.suggest_uniform('collor_jitter', 0, 1.0)
 ...
 #   transforms.Resize((224, 224)),
  ...
  #  transforms.RandomVerticalFlip(),
    ...
    if scheduler_name == 'StepLR':
        ...
        scheduler = optim.lr_scheduler.OneCycleLR(optimizer, max_lr=0.1, steps_per_epoch=len(train_loader), epochs=num_epochs)
    elif scheduler_name == 'CosineAnnealingLR':
        scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=10, eta_min=0, last_epoch=-1)
    elif scheduler_name == 'CosineAnnealingWarmRestarts':
        scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=2, eta_min=0, last_epoch=-1)
...
if __name__ == '__main__':
    ...
    study = optuna.create_study(direction='minimize', pruner=optuna.pruners.MedianPruner(), study_name='optuna_train_!', storage='sqlite:///ChestX.db', load_if_exists=True)
